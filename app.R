...
server <- function(input, output, session) {
  ...
  scrapeLegoData <- function(number){
    ...
    if (legoExists)
      ...
      return(list(imgUrl, lego.setNumber, lego.setName, lego.setYearReleased, lego.setNumOfPieces))
      }
    else{
  ...
  dateToString <- function(date){
    return(paste0(month(date), "/", day(date), "/", substr(as.character(year(date)), start=3, stop = 4)))
  }
  ...
  getCovidFromDate <- function(date){
    ...
    newCases <- pull(polandCases, dateToString(date - 1)) - pull(polandCases, dateToString(date - 2))
  }

  legoData <- reactive({scrapeLegoData(getCovidFromDate(input$chosenDate))})
